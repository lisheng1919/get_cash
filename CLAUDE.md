...
个人量化套利中枢系统 — Python 3.10+, 集成四个低风险套利策略：可转债打新、可转债配债、节假日逆回购、LOF底仓套利。SQLite持久化，APScheduler调度，miniQMT+xtquant执行交易（V1.3+）。
...
**数据层** (`data/`)：`models.py` 定义10张表DDL（`init_db`建表），`storage.py` 封装CRUD（接收`sqlite3.Connection`，不持有全局连接），`collector.py` 主/备源容灾切换（连续失败3次切备用源，状态记入`data_source_status`表）。

**调度层** (`scheduler/`)：`calendar.py` 内存中维护节假日集合，从SQLite加载。`scheduler.py` 包装APScheduler，所有job自动判断交易日（非交易日跳过）。

**策略层** (`strategies/`)：`base.py` 定义抽象基类。四个策略独立实现，通过config.yaml的`strategies.*.enabled`控制启停。LOF溢价监控拆为子包：`premium.py`(计算) → `filter.py`(过滤) → `signal.py`(防抖信号)。
...
- `lof_premium` 策略未接入 `main.py`（无注册、无interval job）
...
`config.yaml` 必须包含 `strategies` 和 `notify` 两个顶层段。关键配置项见 `config_loader.py` 的校验逻辑和 `config.yaml` 的注释。通知密钥（serverchan_key、webhook）通过配置文件管理，不要硬编码。

## Testing

测试使用 `:memory:` SQLite + `unittest.mock`。无 `conftest.py`，每个测试模块自行创建存储实例。63个测试用例覆盖所有已实现模块。

## Docs

完整设计文档在 `docs/superpowers/specs/2026-04-29-lof-premium-arbitrage-design.md`，实现计划在 `docs/superpowers/plans/2026-04-29-get-cash-implementation.md`。
